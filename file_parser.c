...
//        printf("inside file parser literal is %s %s \n",tokens[1],tokens[2]);
    ...
    if(strcmp(ins->opcode, "ADD") == 0 || strcmp(ins->opcode, "SUB") == 0 ||strcmp(ins->opcode, "MUL") == 0 || strcmp(ins->opcode, "AND") == 0 || strcmp(ins->opcode, "OR") == 0 || strcmp(ins->opcode, "EX-OR") == 0) {
        ins->rd = get_num_from_string(tokens[1]);
        ins->rs1 = get_num_from_string(tokens[2]);
    ...
    if(strcmp(ins->opcode,"ADDL") == 0){
        ins->rd = get_num_from_string(tokens[1]);
        ins->rs1 = get_num_from_string(tokens[2]);
//        printf("inside file parser ADDL literal is %s %s \n",tokens[2],tokens[3]);
        ins->imm = get_num_from_string(tokens[3]);
    }
    if(strcmp(ins->opcode,"SUBL") == 0){
        ins->rd = get_num_from_string(tokens[1]);
        ins->rs1 = get_num_from_string(tokens[2]);
//        printf("inside file parser SUBL literal is %s %s \n",tokens[2],tokens[3]);
        ins->imm = get_num_from_string(tokens[3]);
    }
